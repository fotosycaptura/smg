...
class tumanhwas_com:
    ...
    def __get_capitulos(self, url_del_manga, nombre_manga) -> list:
        ...
        listado = []
        for capitulo in capitulos.select('a'):
            nombre_capitulo = capitulo.getText().replace(nombre_manga, '').strip()
            ...
            url = capitulo['href']
            nombre_float = int(float(nombre_capitulo))
            listado.append([str(nombre_float).zfill(4), url])
        return listado
    
    def descargar_solo(self, url_manga):
                Varios = utilerias.utilerias(self.version)
                if(url_manga.startswith("https://tumanhwas.com/manga/")):
